...
% Preamble.
fprintf(fid, '\n');
fprintf(fid, 'Set oModule = oDesign.GetModule("RadField") _\n');
...
fprintf(fid, '    "NumUCells:=", "%f%s", "NumVCells:=", "%f%s", _\n', NoU, NoV);
...
fprintf(fid, '    "UseScanAngle:=", "%s", _\n', 'true');

fprintf(fid, '    "ScanAnglePhi:=", "%fdeg", _\n', Phis);
fprintf(fid, '    "ScanAngleTheta:=", "%fdeg", _\n', Thetas);
